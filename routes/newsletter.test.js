...
describe('Newsletter', () => {
    ...
    it('send a signup email to msdb', async () => {
        ...
        const response = await request.post("/api/newsletter/signup").send(dummySubscriber);
    ...
    it('confirms the subscription', async () => {
        ...
        //bad email
        const response1 = await request.get("/api/newsletter/confirm").query({ email: 'nonexistent@gmail.com' });
        expect(response1.body.error).toEqual("Subscription was unsuccessful");

        //good email, no conf num
        const response2 = await request.get("/api/newsletter/confirm").query({ email: 'massshootingdatabase@gmail.com' });
