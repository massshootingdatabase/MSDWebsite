...
describe("Authorization and Authentication Requests", () => {
  ...
  it("Successfully registers a user", async () => {
    const response0 = await request
      ...
      .send({ email: "dummyfailure@gmail.com" });
  ...
  it("Succesfully logs in the user", async () => {
    ...
    const response0 = await request
      .post("/api/auth/login")
      .send({ email: "anytihgn@aol.com" });
    expect(response0.statusCode).toBe(400);
    expect(response0.body.error).toEqual(
  ...
  it("Sends the forgot password email", async () => {
    const response = await request
      ...
      .send({ email: "massshootingdatabase@gmail.com" });
    expect(response.statusCode).toBe(200);
  });
