...
describe('Incident insertion requests', () => {
    ...
    it('Gets an incident by id', async () => {
        ...
        const response = await request.post("/api/incidents/get").send(dummyIncident);
        ...
        const response1 = await request.post("/api/incidents/get").send(invalidIncident);
    ...
    it('Creates a new incident', async () => {
        ...
        const response1 = await request.post("/api/incidents/create").send(dummyInc).set('Authorization', auth);
        expect(response1.statusCode).toBe(200);

        const insertedIncident = await Incident.findOne({description: dummyInc.description});
